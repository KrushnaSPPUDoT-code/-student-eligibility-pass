/**
 * Deploy the Student Pass contract to a Midnight network
...
import { deriveSecretKey } from './student-pass-utils';

// @ts-expect-error Required for wallet sync
globalThis.WebSocket = WebSocket;

// Identifier under which this contract's private state is stored. The private
// state holds the student's secret key (a private witness).
...
if (!fs.existsSync(path.join(studentPassZkConfigPath, 'contract', 'index.js'))) {
  console.error('\n❌ Contract not compiled! Run: npm run compile\n');
  process.exit(1);
}

const compiledContract = StudentPassContract;
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log('║       Deploy Student Pass to ' + network.padEnd(14) + '                 ║');
  console.log('║       Private Student Eligibility Pass — Midnight            ║');
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
