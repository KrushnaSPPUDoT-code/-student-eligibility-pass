// Wallet construction + sync-state restore for the Private Student
// Eligibility Pass.

import { Buffer } from 'buffer';

// Ledger types come from the midnight-js-protocol barrel, which re-exports
// ledger-v8 (8.1.0) under a stable subpath.
import * as ledger from '@midnight-ntwrk/midnight-js-protocol/ledger';
import { unshieldedToken } from '@midnight-ntwrk/midnight-js-protocol/ledger';
import { setNetworkId, getNetworkId } from '@midnight-ntwrk/midnight-js-network-id';
// As of Midnight.js 4.1.x / ledger-v8 8.1.0 the wallet SDK is consolidated behind
// the single @midnight-ntwrk/wallet-sdk barrel.
import {
  WalletFacade,
  DustWallet,
  HDWallet,
...
export async function persistWalletState(
  ...
  for (const kind of CHILD_KINDS) {
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      process.stdout.write(`  ⚠ Could not serialize ${kind} wallet state (${msg}); next run will re-sync.\n`);
    }
  }
